...
export default class BaseEntity {
     ...
     */
    async findOne<T>(condition: any, project: any = {}, options: any = {}) {
     ...
     */
    async findMany<T>(condition: any, project: any = {}) {
        condition.isDelete = false;
     ...
     * @params condition
     */
    async distinct(key: string, condition: any) {
        condition.isDelete = false;
     ...
     */
    async editEntity<T>(condition: any, payload: any, options: any = {}) {
        condition.isDelete = false;
        if (options.multi) {
            ...
            let updatedData: any = await this.model.findOneAndUpdate(condition, payload, options).lean().exec();
     ...
     */
    async updateEntity<T>(condition: any, payload: any, options: any = {}) {
        condition.isDelete = false;
        if (options.multi) {
            ...
            if (typeof options.new === 'undefined') options.new = true;
            let updatedData: any = await this.model.findOneAndUpdate(condition, { $set: payload }, options).lean().exec();
            if (updatedData) return { type: 'SINGLE', data: updatedData };
            else return { type: 'SINGLE' };
        }
    }
